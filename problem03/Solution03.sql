...
), GET_LOG_TIME (LOG_TKTM, USR_NO, MENU_NM) AS (                 -- Get access times of the most accessed menu of each users (resolve tied menu)
	SELECT M.LOG_TKTM, M.USR_NO, M.MENU_NM 
	FROM MENU_LOG AS M, MAX_MENU AS K
	WHERE M.USR_NO = K.USR_NO AND K.MENU_NM =M.MENU_NM
), GET_MAX_LOG_TIME (M_TKTM, USR_NO) AS (                   -- Pick the recent and most accessed menu of each users
