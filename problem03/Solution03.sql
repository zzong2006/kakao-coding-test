WITH RECENT_RSDT (USR_NO, RSDT_NO) AS (               -- Find recent rsdt of users 
	...
	FROM (SELECT MAX(LOG_ID) AS MAX_ID, USR_NO FROM USR_INFO_CHG_LOG WHERE RSDT_NO <> "" GROUP BY USR_NO ORDER BY NULL) A,
	 USR_INFO_CHG_LOG 
	WHERE A.MAX_ID=USR_INFO_CHG_LOG.LOG_ID
), RECENT_MCCO (USR_NO, MCCO_NM) AS (                  -- Find recent mcco of users
	SELECT A.USR_NO, MCCO_NM
	FROM (SELECT MAX(LOG_ID) AS MAX_ID, USR_NO FROM USR_INFO_CHG_LOG WHERE MCCO_NM <> "" GROUP BY USR_NO ORDER BY NULL) A,
	 USR_INFO_CHG_LOG 
	WHERE A.MAX_ID=USR_INFO_CHG_LOG.LOG_ID
...
LEFT JOIN 
	(SELECT USERS.USR_NO, IFNULL(REG_DAY.RegDay, '-') AS REGISTER_DAY
	 FROM USERS LEFT OUTER JOIN (SELECT USR_NO, SUBSTR(MIN(LOG_TKTM),1, 8) AS RegDay FROM USR_INFO_CHG_LOG GROUP BY USR_NO  ORDER BY NULL) AS REG_DAY
...
LEFT JOIN
	...
	FROM (SELECT MAX(LOG_ID) AS RECENT_ID FROM MENU_LOG 
		WHERE MENU_NM != 'logout' AND MENU_NM != 'login' GROUP BY USR_NO  ORDER BY NULL) AS T, MENU_LOG 
